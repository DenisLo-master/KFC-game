...
test.describe('Phase 3 mobile HUD acceptance', () => {
  test('validates responsive HUD, controls, service view, and pause/reset with one page lifecycle', async ({ page }) => {
    ...
    expect(await dock.evaluate((element) => element.scrollWidth > element.clientWidth)).toBe(false);
